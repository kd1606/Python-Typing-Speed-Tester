...
def game():
    ...
    def check_result():
        ...
        if len(words[word])>=len(answer): 
            ...
            for i in answer: #take shoter sentence
                if i == words[word][j]:
                    pass
                else:
                    error+=1
    ...
    x3 = Button(window, text="Submit!", font="times 20",bg="MediumOrchid1",command=check_result)
    x3.place(x=220, y=350)

    entry = Text(window)
    entry.place(x=100, y=180, height=150, width=350)

    b2 = Button(window, text="Done", font="times 13", bg="MediumOrchid1", width=12, command=finish)
    b2.place(x=155, y=420)

    b3= Button(window, text="Try Again", font="times 13", bg="MediumOrchid1", width=12, command=game)
...
x1 = Label(root, text="Let's test your typing speed!", bg="thistle1", fg="black", font="times 20")
x1.place(x=100, y=50)

b1 = Button(root, text="Go!", width=12, bg="MediumOrchid1", font="times 20", command=game)
b1.place(x=150, y=120)

hs_text = Label(root, text="Highscore", width=12, bg="MediumOrchid1", font="times 35")
hs_text.place(x=90, y=240)

hs = int(hs_file.readline())
hs_val = Label(root, text=str(hs)+" WPM", width=12, fg="black", bg="MediumOrchid1", font="times 30")
